...
exports.findOne = async (req, res, next) => {
    try{
        ...
        return next(
            new ApiError(
                500,
                `Error retrieving contact woth id=${req.params.id}`
            )
        );
    }
};
...
exports.update = async (req, res, next) => {
    if (Object.keys(req.body).length === 0) {
        return next(new ApiError("Data to update can not be empty"));
    }
    try{
        const contactService = new ContactService(MongoDB.client);
...
exports.findAllFavorite = async (_req, res, next) => {
    try {
        ...
        return next(
            new ApiError(
                500,
                "An error occorred while retrieving favorite conntacts"
            )
        );
    }
};